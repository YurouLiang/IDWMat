...
b =148080;
theta =69.1; 
data=[yizhi(:,2:4),yizhi(:,12)];
data(:,4)=data(:,4)+(0.6./100).*data(:,3);
K=10;N=15;a=2;g=1;G=500;
    ...
    indices = crossvalind('Kfold', row, K);
     ...
     for t = 1:length(textdata)
         test_point = textdata(t, :);
          dis = sqrt(((traindata(:,1) - test_point(:,1)) .* cosd(theta) + (traindata(:,2) - test_point(:,2)) .* sind(theta)).^2 / a^2 + ...
             ((traindata(:,1) - test_point(:,1)) .* sind(theta) - (traindata(:,2) - test_point(:,2)) .* cosd(theta)).^2 / b^2); 
         [dis,j]=sort(dis);
         dis=dis(1:N);
         j=j(1:N);
         f=traindata(j,4);
...
tempMAPE_SERIIDW(g)=cross_val_errorsMAPE;
tempR2_SERIIDW(g)=cross_val_errorsR2;
g=g+1;
end
RMSE=min(tempRMSE);
MAE=min(tempMAE);
MAX=min(tempMAX);
MIN=min(tempMIN);
MAPE=min(tempMAPE_SERIIDW);
R2=min(tempR2_SERIIDW);
% 打印结果
fprintf('RMSE: %.4f\n', RMSE);
fprintf('MAPE: %.4f%%\n',MAPE);
fprintf('MAE: %.4f\n', MAE);
